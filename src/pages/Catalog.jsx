...
//           // roughness: 0.05,
//           // metalness: 0.1,
//           // transmission: 0.0, // Zero transmission as you requested
//           // transparent: false,
//           // opacity: 1.0,
//           // ior: 2.417,
//           // reflectivity: 1.0,
...
//           roughness: 0.00,
//           metalness: 0.1,
//           transmission: 0.98, // Zero transmission as you requested
//           transparent: false,
//           opacity: 1.0,
//           ior: 2.417,
//           reflectivity: 1.0,
...
//         // Dynamic Fix: Check if it matches the main stone OR contains keywords indicating it's a gem/diamond
//         if (
...
function DynamicRing({ modelPath, stoneName, config }) {
  ...
  React.useEffect(() => {
    clonedScene.traverse((child) => {
      if (child.isMesh) {
        const nameLower = child.name.toLowerCase();
        // Material ka naam bhi check karne ke liye safe fallbacks
        const matNameLower = child.material && child.material.name ? child.material.name.toLowerCase() : "";
        
        // --- FIXED DETECTION LOGIC ---
        // 1. Strict match with configured stoneName
        // 2. Mesh name contains diamond/gem/stone/crystal
        // 3. Original material name contains diamond/gem/stone/crystal/glass
        if (
          child.name === stoneName || 
          nameLower.includes("diamond") || 
          nameLower.includes("gem") || 
          ...
          >
            ...
            <div className="w-full max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-12 gap-12 items-center pt-8">
              ...
              <div className="lg:col-span-7 h-[400px] lg:h-[550px] w-full flex items-center justify-center relative overflow-hidden rounded-2xl bg-gradient-to-b from-neutral-900/20 to-transparent border border-neutral-800/40 cursor-grab active:cursor-grabbing">
                ...
                {/* FIX 1: Moving the dynamic key directly onto the Canvas object forces 
                    the entire stage system to recalculate cleanly per unique ring select. */}
